...
export default function LoginPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    // TODO: replace with real auth (Clerk)
...
.xo-auth-logo { font-size: 20px; font-weight: 600; color: var(--ink); margin-bottom: 32px; letter-spacing: -0.02em; display: block; }
...
.xo-btn-auth {
  ...
  transition: background 0.2s; margin-top: 8px; display: block; text-align: center;
}
.xo-btn-auth:hover:not(:disabled) { background: var(--green); }
.xo-btn-auth:disabled { opacity: 0.6; cursor: not-allowed; }
.xo-social {
    ...
    <>
      ...
      <div className="xo-auth-wrap">
        <div className="xo-auth-card fade-in">
          ...
          <button className="xo-social" onClick={() => router.push("/app")}>
          ...
          <form onSubmit={handleSubmit}>
            <div className="xo-field">
              <label>Email</label>
              <input type="email" placeholder="you@yoursite.com" required />
            </div>
            <div className="xo-field">
            ...
            <button type="submit" className="xo-btn-auth" disabled={loading}>
